 ...
 * 把我们自己的大脑一起挡在外面（自我 DoS），而前者不影响持有正确 token 的调用。
 *
 * **它提供的不是速率上限**：`delay()` 是挂起不是阻塞，计数虽然同步但延迟各自计算，
 * 攻击者开 N 条并发连接就能拿到 N 次/2s。所以这层只惩罚**串行**猜测，纯属纵深防御——
 * 真正扛住暴力枚举的是 128 bit 的 token 本身。别把它当成限流器来依赖。
 */
