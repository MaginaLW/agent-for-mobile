...
class StaleReconfirmGuard(
    ...
    companion object {
        ...
        /**
         * 计数键。B 道当初钉了三条不变量、把键的定义交给 A 道；前两条是关于键本身的，仍然有效：
         *
         * 1. **不得把两个不同的语义动作并成一次**（否则第二个动作会莫名其妙被拒）。
         * 2. **不得把同一个语义动作的重试拆成多次**（否则限次形同虚设，退化回无界循环）。
         *
         * 第三条原文是"停下时走 `[AWAIT_CONFIRM]`，不是静默放弃"，**已于 2026-08-02 作废**：
         * 用户重新拍板维持站规「安全失败即终态」，耗尽路径改为按站规报告「结果：失败」，
         * 且**明令不得输出 `[AWAIT_CONFIRM]`**（见 [EXHAUSTED_FALLBACK]）。它本来也不是关于键的。
         * 留在这里只为说明它去哪了——**一条指向已不存在行为的注释比没有注释更坏**，
         * 顺着它去看调用方的人会看到完全相反的东西。
         *
