 ...
 *
 * 批次 2 把审批从"盯着屏幕点悬浮卡"扩成"锁屏通知上也能点"，于是同一次确认**同时有两条
 * 通道**能送进决定。两件事必须机械保证：
 *
 * 1. **先到者赢，且只有一个决定生效。** 否则会出现"卡上点了拒绝、通知上点了允许"这种两个
 *    真人决定互相打架的状态——而 Deny 腿那次假通过已经证明，这类状态的失败形态长得跟成功
 *    一模一样。这里的 CAS 就是那唯一的胜负判定：[decide] 把决定交给 [Pending.complete]，
...
object ConfirmApprovalArbiter {
     ...
     *
     * 它**不**能设置、伪造或跳过真人决定：`allowed` 只能来自真人在卡或通知上的点击，
     * 本方法没有任何"默认允许"的路径。
     */
    @Synchronized
