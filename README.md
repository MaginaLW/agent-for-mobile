...
| M1-T | Android 平板横屏基线（T0-L 画像 + T-L1 pane 探针 + T-L2 P0） | 🟡 进行中——[适配设计](docs/specs/2026-08-23-Android平板适配-design.md)；T0-L v5 已完成，C1a 已在日常双窗形态建立可信来源/只读取证但未通过 T-L1。C1b pure-a11y 的 offline gates、五场景 synthetic host E2E、real isolated host build smoke 与独立复审已通过。fixed SHA `87ac7b45e79bf658ca6e56b697a24f52fdf7381b` 获得的唯一 C1b 授权已执行并[冻结失败](docs/runs/2026-08-28-T-L1-C1b私有ADB启动失败.md)：runner exit 1，private ADB server 未在有界时间内 ready；WER 证明同轮 isolated ADB 六次同签名崩溃。控制流停止于设备发现之前，未执行 `install -r -t`、T0 或两帧采集，无 run/evidence/sidecar，清理无残留且没有重试。下一步回 A 道补可观测性、诊断并固定新 SHA；旧 C1a 与本次 C1b 授权均不可复用。导航/会话/目标区、布局、P0 与执行仍未放行 |
