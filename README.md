...
| M1-T | Android 平板横屏基线（T0-L 画像 + T-L1 pane 探针 + T-L2 P0） | 🟡 进行中——[适配设计](docs/specs/2026-08-23-Android平板适配-design.md)；T0-L v5 已完成，C1a 已在日常双窗形态建立可信来源/只读取证但未通过 T-L1。C1b pure-a11y 已形成 exact 41-file implementation/build-input catalog binding、专用 Debug/Release probe 与 artifact/manifest/DEX/aapt2 证明；全部设备命令绑定随机 loopback private ADB server，default 5037 永不使用。full offline gate 已通过（host coverage 26/26），build-env 27/27、private ADB 16/16、T0 sidecar 7/7、readonly 70/70；五场景 synthetic host E2E 稳定复跑通过，fake ADB 219（211 valid + 8 rejected）、fake Gradle 6、fake signer 10，E2E 内 real ADB/JDK/Gradle executions 0，逃逸 child/listener/side-effect 0。另行 real isolated host build smoke 已通过：受控 JDK/GradleMain 1 次、held-Java ApkSignerTool 1 次、real ADB 0、repository inputs 41；独立复审 P0/P1/P2=0。该 smoke 不构成 fixed-SHA C1b build/install/runner 或真机取证；下一步固定最终 clean HEAD，再取得一次不可复用的 C1b build/install/只读采集授权。导航/会话/目标区、布局、P0 与执行仍未放行 |
