...
| M1-T | Android 平板横屏基线（T0-L 画像 + T-L1 pane 探针 + T-L2 P0） | 🟡 进行中——[适配设计](docs/specs/2026-08-23-Android平板适配-design.md)；T0-L v5 已完成，C1a 已在日常双窗形态建立可信来源/只读取证但未通过 T-L1。fixed SHA `87ac7b45e79bf658ca6e56b697a24f52fdf7381b` 的唯一 C1b 授权已[冻结失败](docs/runs/2026-08-28-T-L1-C1b私有ADB启动失败.md)：private ADB 未 ready，控制流停止于设备发现之前，未 install/T0/c1/c2，清理无残留且没有重试。2026-08-29 的 42-input 离线修复已将 server listen 固定为 `-L tcp:localhost:<port>`，client/environment/listener proof 保持 numeric `127.0.0.1`，并为 run_id 前失败加入 post-cleanup 原子发布的 bounded structured attempt evidence；最终离线结果为 host/full gate 29/29、observation 49/49（coverage 89/89）、七场景 E2E、private ADB 22/22、readonly 74/74、attempt schema/cross-binding 51/51，当前独审 P0/P1/P2=0，候选固定为 `77473af5223d76b00bf4dbbf33cf44090fde635c`。该 SHA 的一次 42-input real isolated host build smoke 随后[冻结失败](docs/runs/2026-08-29-T-L1-C1b-42-input-real-build-smoke失败.md)：GradleMain `1` 后因一次性 helper 漏载 validator 而在 artifact proof strict JSON reader fail-closed，ApkSigner/AAPT2/real ADB/设备发现/install/T0/采集均为 `0`，retry `0` 且契约内 runtime/build residue 为 `0`。生成过临时 APK/proof 不等于 smoke 通过；该 SHA 不得进入设备授权。下一候选须回 A 道修正并审查 helper、重新固定 clean SHA，再单独取得一次 real build smoke 授权；旧 C1a 与两次已消费的 C1b 授权均不可复用。导航/会话/目标区、布局、P0 与执行仍未放行 |
