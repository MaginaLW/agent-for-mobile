...
- **手机 C 道已按用户 08-23 决定暂停，后续真机统一使用 Android 平板。** 手机批次 4 保持 0/4，八条替代 C 原样冻结，不改写成平板失败。平板任务已从 `67ef56c` 派生 `codex/tablet-intake`：先做 T0 只读画像/姿态/窗口探测，零输入、零发送；T0 通过前不开四腿，也不直接放开当前 `h>w`/手机比例门。
