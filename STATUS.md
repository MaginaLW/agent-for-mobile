...
- **批次 4 当前基线为 `d36e3d2`，分支 `claude/serene-faraday-42d1fb`；main 行为仍未改变。** 它修掉 `3ed077d` 真机暴露的 fresh-title 日期误选，并把 final-title 脱敏同源证据升为成功硬门；独立复审 Approved，离线 gate 全绿：dispatch 58、runner 144、Debug/Release/assembleDebug、凭据扫描。
...
- **当前唯一下一步是 A 道修 runner 假阴性**：保留原始 `ui_find.text` 与 marker 的逐字绑定，`query_normalized` 与每个 match 只比较 gateway 同源字段；真实 producer fixture 先 RED，再完整 gate/独立复审/新 SHA，最后另建 clean 固定 C 从头四腿。用户 08-11 接受的 Codex 0.147 `view_image` 有界 residual 继续有效，无需再问。
