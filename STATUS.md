...
- **批次 4 当前待验基线为 `f0a767335e70aa99ed0fc242a1217978600435af`，分支 `claude/serene-faraday-42d1fb`；main 行为仍未改变。** runner 保留原始 `ui_find.text` 与本腿 marker 的逐字绑定，canonical 只比较 gateway 的非空同源字段，全部 match 逐字相等且消息区几何仍独立 fail-closed；独立复审 Approved，完整 gate 全绿：dispatch 58/58、Debug 510/510、Release 407/407、assembleDebug、runner 146/146、凭据扫描。
...
- **三条旧 C 均冻结且不复用**：占位通道 run `20260809T203420-6cf147532b9f` 只算基础设施失败；`3ed077d` run `20260811T202517-0e176d3f08b9` 在 Allow 因标题误选正确 fail-closed、未发送；`d36e3d2` run `20260811T215340-d3eb4c2bdeaf` 的标题硬门已成立且消息实际发送，却被 runner 自己去连字符、gateway 保留连字符的重复归一误判。后三腿均未运行，批次 4 仍 **0/4、未判定**。
- **当前唯一下一步是置顶的 clean 固定 C task `019ff195-0fde-7eb2-ac1a-88ee11cc1a2d`**：已钉 `f0a7673`，HEAD/clean/`local.properties` 预热均通过，正等待用户回复“已就位”；随后只在该任务从头跑 Allow → Stale → Deny → Reentry（75s），任一失败即冻结，旧 task/run 永不复用。用户 08-11 接受的 Codex 0.147 `view_image` 有界 residual 继续有效，无需再问。
