...
- **当前唯一下一步是置顶的全新固定 C task `019ffbc0-f5a7-7701-9a86-0bf3d07242bf`**：已钉 `636048a`，HEAD/clean/ignored `local.properties` 预热通过，未构建/安装/adb/runner，正等待用户在该任务回复“已就位”；随后只跑唯一 build + Provision 安装 + Allow → Stale → Deny → Reentry（75s），任一失败立即冻结。所有旧 task/run 永不复用；Codex 0.147 `view_image` 有界 residual 继续有效。
