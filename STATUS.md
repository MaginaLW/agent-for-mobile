...
- **手机 C 道已按用户 08-23 决定暂停，后续真机统一使用 Android 平板。** 手机批次 4 保持 0/4，八条替代 C 原样冻结，不改写成平板失败。平板 T0 只读入场候选已钉 `451b1bf6f32dbc154978bd3b6f02921fbd04f463`（`codex/tablet-intake`，从 `67ef56c` 派生），fake-adb 16/16、AST/diff/凭据扫描与独审通过；首次真机入口识别到 0 台 ADB 设备，exit 1 后 fail-closed 停止，未生成 run/profile、未重试。待平板开启 USB 调试并授权本电脑；行为不合 main，T0 通过前不开四腿，也不直接放开当前 `h>w`/手机比例门。
