...
3. **同一现象连撞两轮还没定位，第三轮之前先查官方文档/搜网**，不要接着调参数。反复试参数是在"用真机时间换本该几分钟搜到的知识"——2026-08-02 批次 2 锁屏通知烧掉三轮真机，一次搜索就查出 `CATEGORY_CALL` 是"申请"不是"授予"、而 `CallStyle` 我们结构上用不了（详见 [android/common.md](docs/knowledge/android/common.md)）。厂商 ROM 文档常抓不到正文，那就明确记下"这条线指望不上文档"，转做单变量实验。
4. 构建日志、logcat、长命令输出先落盘（重定向到文件）再 grep/尾读，不整段读入。
5. 大文件按行区间读，不整读。
6. 会话收尾两件事：更新 STATUS.md；新踩的坑写入对应 knowledge 册。
