...
## 监督式 runner 离线套件对机器负载极其敏感（2026-07-26）

39 条用例每条都要起 pwsh 子进程、造一个临时仓库副本、再经假 adb/dispatch 走完整腿，单条实测 8–9 秒，整轮 9–17 分钟。**它的失败率是机器状态的函数，不是代码的函数**：同一个提交当天先跑出 38/38，几小时后（累积了 3–4 个常驻 gradle daemon、若干被 kill 的跑测残留目录、可用内存降到 ~3GB）跑出 11/38，失败全是 `runner 离线测试超时` 或 `读取 gateway 审计游标 超时`——都是**进程启动**超时，没有一条是断言失败。

- **判别方法**：怀疑回归时先 `git stash` 全部改动、在上一个提交上原样跑一遍。基线同样大面积超时就是机器，不是代码。别靠单跑几条来下结论——单跑必过，没有判别力。
- **别在跑这套件时并行做任何事**（尤其 gradle）：会直接把它压过阈值，制造出成片的假失败。
- `Invoke-FixtureRunner` 的墙钟预算原为 15 秒，对 8–9 秒的真实耗时余量太薄，已放宽到 60 秒。**它是防挂死的保险，不是性能断言**，放宽不削弱任何安全属性。
- 收尾清理：`app/gradlew.bat --stop` + 删掉 `%TEMP%\agent-mobile-p0-*` 残留目录（被 kill 的跑测不会自清）。
