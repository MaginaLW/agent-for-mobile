...
- **离线全绿的 provision 脚本首次接真机大概率还会炸；别把离线通过当真机通过**（2026-07-23 首次真机 P0 provision 实锤，连续踩 4 个坑，均只在真机才现形）：
  ...
  20. **确认卡截图拍不到卡本身——而这张 PNG 是监督式跑测唯一的现场证据**（2026-07-26 Allow 腿实锤）。翻旧证据（`docs/runs/evidence/<run>/allow/confirmation.png`）直接看图即可确认：图里是完整的微信会话页、输入框里躺着本腿 marker，**没有任何卡的痕迹**；而同一次流程的 manifest 显示确认状态确实走到了 `allowed`，即人确实在卡上点了「允许本次」。
