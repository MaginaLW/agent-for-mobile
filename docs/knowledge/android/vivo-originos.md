...
- **`adb install -r` 会被 vivo 系统安装确认页同步拦住**（Android 16 / OriginOS，2026-08-22 实测）：命令发起约 2s 后拉起 `com.android.packageinstaller/.PackageInterceptActivity`，在真人点击继续安装前 adb 进程一直不返回；runner 的 120s 超时会把整轮冻结为 0 腿 setup fail。开跑前必须让用户盯住手机，Provision 开始后立即核对并确认本项目 debug APK；“设备已授权、`adb get-state=device`”不等于“无人值守安装可用”。超时后用 `dumpsys package` 的 `lastUpdateTime` 区分安装是否真的完成，不能只看包路径存在（可能是旧包）。
- **换开发机/用户配置后先验 debug 签名，再烧一次 `install -r`**（2026-08-22 实锤）：Gradle 在缺少 `~/.android/debug.keystore` 时会新生成 key，同包名旧 debug APK 不能被新 key 覆盖。vivo 安装页可能只短暂返回、不给可见错误；runner helper 又会丢弃 adb stderr，只剩笼统“安装失败”。正判据是只读 pull 已安装 `base.apk`，用同一 SDK `apksigner verify --print-certs` 比较旧/新证书 SHA-256；不一致时只能恢复旧 keystore，或经用户明确授权卸载旧包后重装。卸载会清 App 私有数据和系统授权，属于需单独确认的破坏性前置，不能由 runner 猜着做。
...
## 通知与锁屏（2026-08-02，批次 2 逼出来）

AOSP 级的通知资格规则在 [common.md](common.md#通知上不上锁屏categorycall-是申请不是授予)，这里只记 vivo 特有的。

- **`zen_mode` 是真机跑测的隐形混杂变量。** 08-02 第一次锁屏验证撞上 `zen_mode=1`
  ——vivo **定时睡眠模式**自动开启，而审批通道 `mBypassDnd=false`。**跑锁屏相关验证前必须先
  `adb shell settings get global zen_mode` 确认为 0**，别只看"我没手动开免打扰"。
- **vivo 的锁屏通知有两级开关，且分属不同页面**：应用级「锁屏通知」开关，以及
  「**显示锁屏通知详情**」（设置 → 状态栏与通知 → 管理通知 → 选对应应用）。前者控制显不显示，
  后者控制显示内容还是折叠成"1 条新消息"。**排查时两个都要看**，只确认前者会得出错误结论。
- **OriginOS 有自己的「原子通知」呈现层**，通知在系统 UI 上的形态与 AOSP 不完全一致。
  🔵 是否影响锁屏可见性未实测。
- **vivo 开放平台的开发者文档拿不到开发者级的锁屏规范。** `dev.vivo.com.cn/documentCenter`
  的页面是 JS 渲染的，抓取只得到站点标题；中文搜索结果几乎全是面向终端用户的设置教程。
  **结论：这条线上不要指望文档，直接做单变量真机实验更快。** 官方文档只查到通知栏样式规范
  （图标/应用名一致性、悬浮通知 5s 自动消失等），与锁屏可见性无关。
