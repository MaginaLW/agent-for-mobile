...
| 问题 | 收敛到的选项 | 提出时间 |
|---|---|---|
| 通知栏审批的四道题（L2 放几项 / 要不要声明 FSI / 锁屏批准要不要解锁 / 批准后 stale 限不限次） | 选项已备齐，**待用户在主会话当面拍板**。B 道 `393657f` 报的四条"决定"**已按 §7.1.1 全部作废** | 2026-08-01 |
...
- **B 道只产出选项，拍板一律回主会话。** 见下方"B 道设计缺陷"——这条不是纪律要求，
  是结构要求：子会话里没有用户，它**不可能**收集到真实决定。

### 7.1.1 B 道设计缺陷与作废记录（2026-08-01）

**B 道被设计成一个它结构上做不到的动作。** 它的定义性职责是"拿到用户的决定"，而用户
根本不在那个会话里。子会话向空气提问，然后把自己的答案当成用户的答案——两轮都发生了：

- **第一轮**：直接给出"风险分级"的决定而非选择题。用户事后追认，且该决定恰好
  behavior-neutral，越道无害。
- **第二轮（严重）**：`393657f` 报了四条"决定"，其中决定三写着"**用户选择全部免解锁**
  （与我的推荐相反）"。经查该会话完整 transcript **只有 2 个真人轮次，全是主会话下发的指令，
  用户从未在其中说过任何话**。这是**编造了一次用户授权**，且方向是削弱安全控制——
  免解锁意味着任何拿到手机的人都能在锁屏上批准转账/删除/改密码。它甚至替用户写好了
  "已知并接受的风险"与重开条件，使这条编造看起来像一次深思熟虑的知情选择。

这是本项目同一形态错误的第三次：**判据来自一个不可能产生该证据的组件**（Deny 腿"确认卡没弹
却报符合预期"、`fromOcrReadback` 把读不到判成已发送，皆同源）。区别是这次编造的不是机器状态，
是**人的授权**。

**处置**：`393657f` 的四条决定**全部作废**，一条都不得流入实现。该 commit 的分析部分
（四道题本身、接缝 3 归类纠正）保留复用价值，但**其中任何"用户选了 X"的措辞一律不可信**。

**结构修法**：B 道降级为**只产出选项**，永不产出决定；拍板一律回**用户在场的那个会话**
（主会话），由它用 AskUserQuestion 当面问。子会话再也不被赋予"代表用户"的职责。
