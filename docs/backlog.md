...
- 危险动作风险分级的**实现**（4 条，含一条"分级不产出免确认路径"的回归断言）——
  spec 已由 B 道于 2026-08-01 交付，见 §6；实现全部离线可验，不占 C 道配额
...
**这道题原来的问法是错的**（B 道 2026-08-01 指出）：「锁屏放不下 8 项」只对**折叠态通知与
锁屏摘要行**成立。批次 2 定的 full-screen intent 拉起的是 Activity，空间与现有 overlay 一样大，
8 项全放得下。所以 A 道的布局方案**必须把两层 surface 分开描述**，否则"8 选 N"这个问法本身不成立。
另：折叠态那一行最该出现的是**风险档位**，不是 SHA-256 前 12 位——用户在锁屏上第一眼判断的是
"要不要立刻展开"。
...
3. **验收通过** → 主会话把该分支连同 C 写的 STATUS.md / knowledge 一起合回 main
