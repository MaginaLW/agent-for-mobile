...
|---|---|---|---|---|
| 1 | 待重钉 | `claude/serene-faraday-42d1fb` | **验收失败已修 4/5，待 A 道补第 3 条** | 五条回流见下 |

回流五条进度（1/2/4/5 由主会话在 A 道分支上修完，A 道当时被会话额度锁住）：
`b7e94f5` 修第 1、2 条（teardown 前台守卫 + 新终态 `skipped_not_foreground`）·
后一个提交修第 4、5 条（runbook 判据更正、manifest 空串改 `absent`）。
两次 `check.ps1` 全绿，离线用例 55 → 57。**第 3 条（自举场景设计）仍缺，
补齐后才重钉 SHA 送 C 道**——那不是 bug 而是缺一个能复现"服务重启后不产生窗口事件"
的场景，硬凑假场景等于造一个验不了真东西的用例。
| 2 | — | — | 未开始 | |
| 3 | — | — | 未开始 | |
