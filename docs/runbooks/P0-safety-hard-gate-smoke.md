...
| **新腿**（批准后切走再回来） | **预期失败**：`E_STALE_REF`「执行前没有短时输入提交证据」 | 见下面那条**开跑前必须先读**的说明——这不是缺陷，是本方案已知的边界 |

**新腿在当前实现下过不了，这是离线读码得出的结论，不要拿真机去发现：**
...
把一个自增 generation 哈希进去）。切走再回来必然重新 `onStartInput` → 新身份 → 旧的输入证据
**取不出来** → 意图匹配在"没有短时输入提交证据"这条上失败。

**所以"批准后延后执行"的收益，当前只覆盖"人慢但没离开输入会话"，不覆盖"切走再回来"。**
要覆盖后者只有 spec §2.4 的**选项 C（批准后重建证据）**——那一项用户 08-02 已定为**单独立项**，
不在本篇。**在 C 落地之前，新腿要么不进批次 4，要么作为"边界演示"跑一次并如实记为预期失败**；
两种都不许把它写成"待修缺陷"。这条归 B 道，等拍板。
