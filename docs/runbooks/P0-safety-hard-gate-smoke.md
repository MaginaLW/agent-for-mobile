...
2. 在系统输入法设置中启用「执行网关」输入法，并把它切换为**当前输入法**。进入微信文件传输助手并聚焦输入框，现场核对当前生效的确为执行网关；`type_text` 的 IME fallback 和 Enter fallback 都依赖 active IME，仅“已启用但未切换”不满足前置条件。记住原输入法，三腿结束或中止后按 §4 恢复。网关进程被杀或重装会**静默回退当前输入法**（2026-07-22 实锤），每腿启动前要重新确认当前输入法仍为「执行网关」。
3. 授予“显示悬浮窗”权限；在 vivo/OriginOS 的应用权限中另行允许「后台弹出界面」。两者不是同一权限，缺后者时网关在微信前台可能无法显示确认卡。
4. 将网关加入电池白名单并允许后台高耗电，避免无障碍或前台服务被系统静默回收；并在最近任务里把「执行网关」卡片**锁定**——进程被清理会中断腿并连带回退输入法（2026-07-22 实锤）。
5. 回到网关主界面点「启动网关服务」，确认页面显示服务运行、无障碍已开启、悬浮窗已授权。
6. 微信已登录，能进入「文件传输助手」；三腿开始前输入框必须为空。微信「设置→聊天→使用回车键发送消息」必须开启（Enter 发送链依赖它）。不要预先发送或复制任务卡中的三条测试文本。
...
每腿命令启动前，手机操作人都要先打开微信「文件传输助手」、目视确认输入框为空，然后**点一下输入框使其聚焦**（当前输入法为「执行网关」；它是注入型 IME、无可见键盘窗口，聚焦以输入框光标为准，`keyboard.visible=false` 属正常，2026-07-22 实锤），保持屏幕常亮直到确认卡出现。若拒绝腿等前序步骤留下了未发送文本，记录该残留，但不得手动按 Enter、点发送或换路发送；任务卡会用无 ref 的 `type_text(mode="replace")` 覆盖旧内容（本页树空且空白输入框无 OCR 锚点，`ui_find` 拿不到输入框 ref，2026-07-22 实锤）。每腿都必须以 `type_text` 返回的 `committed=true`、`ime_commit` 通道及只读 `ui_find` 命中测试文本为首要输入证据，必要时再由模型做只读感知。
