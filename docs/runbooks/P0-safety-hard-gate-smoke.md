...
|---|---|---|---|
| 重建成功 | `OK` | 现在框里的内容与卡上给人看过的那份逐位相同 | 新腿通过 |
| 不匹配 | `E_STALE_REF` | 读到了，但内容在人批准之后被改过 | **正确拦截**，不是缺陷 |
| 判不了 | `E_VERIFY_FAIL` | 读不回来（OCR 一个字没出 / 通道不可用） | **也不放行**，但要与上一行分开记——它说明取证链坏了，不是内容被换了 |

**基线永远是卡上那份 sha256**，不是任何一次读回来的串（发送后验踩过这个假阳性：基线取了
Enter 前那次 OCR 读回的噪声串，于是"读回来的和读回来的一样"平凡成立）。离线由
`EvidenceRebuildPolicyTest` 逐条钉住，含一条"读回来的串永远不能当自己的基线"的反例。
