...
5. **开跑前确认 USB 线材与端口。** 2026-08-08 第三跑 USB 掉线两次，**两次都在跑测中途**：
   第二次直接把新腿的拉回步骤打成 `setup-fail`，连带整轮清理失败。**这不是操作问题**，
   所以它属于物理前置而不是现场注意——换一根确定可靠的线、插主板直连口，别用扩展坞。
   新腿尤其吃这一条：它中途要经 adb 把微信拉回来，**掉线正好落在那一步上**。
