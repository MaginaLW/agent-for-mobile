...
**跑前多一条物理前置（2026-08-08 新增，第一次跑就是栽在它上面）**：
`configs/gateway-mcp.json` 里必须有 **`"timeout": 420000`**。HTTP MCP 传输的「首字节」
计时器默认 **60 秒**，而这条腿的 `press_key` 要阻塞到 400s 量级——**缺这个字段必被砍，
且现场看到的是「客户端超时、无响应体、无错误码」，与「功能坏了」分不开**。
这个配置是 gitignored 的私密文件、不随 checkout 过来，所以 `dispatch` 开跑前会校验它，
缺席或过小直接拒跑。另一道 300s 的空闲看门狗由**网关在阻塞期间发心跳**顶住（见下）。
