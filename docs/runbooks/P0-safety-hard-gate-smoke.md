...
**被拦下的腿会把 marker 留在输入框，因此它后面那一腿必然要人工清一次框**（2026-07-31 实锤：`-Legs Stale,Deny` 一次跑完，Stale 通过后 Deny 在预检被拦，残留 `P0STALE-…`）。这不是缺陷而是设计的直接推论：Stale/Deny 两腿的定义就是**发送被拦**，文字自然还在框里；而 runner 不能自己去清——那就成了「工具碰微信」，违背本节开头「人工预置状态、工具不碰微信」的原则。被拦下的腿还会**把键盘留在屏上**，这会连带触发预检的另一条判据 `输入法窗口占屏`——不是多余的挑剔：候选区是**固定几何**（如 `[302,2637,957,2727]`），键盘一支起来微信输入栏整体上移（实测移到 y≈1720），此时那块固定区域量的是键盘而不是输入栏，`empty` 会得出**看似为空的错误结论**。所以清框之外还要收起键盘。所以：**Allow 腿后面可以直接接下一腿**（消息发出去了，框是空的），**Stale/Deny 后面必须停下来让人清框并收键盘**。想一次跑多腿就把 Allow 放最前（`Allow→Stale→Deny` 的固定顺序正是这个道理），Stale 与 Deny 之间预留一次人工清框。预检会零 token 把这件事在派单之前拦下并回显残留文字，代价只有几十秒。
