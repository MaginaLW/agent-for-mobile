...
**⚠️ 「状态栏排除」这条判据目前是「未覆盖」，不是「通过」（2026-08-09 第五跑）**：
新腿那跑 `sysrej=0+0`、`fg=-1+0`——**a11y 一个前台元素都没给，窗口归属那道一次都没触发**，
于是第四跑那个失败形态（a11y 把状态栏网速当标题）在那一跑里**物理上不可能发生**，
与修复灵不灵无关。参照锁屏两条的处理方式：**从未验过、不当成通过、写明为什么。**

- **收口判据**：`sysrej` 至少在某一跑里 `> 0`，这条才算验到；否则验收单如实记「未覆盖」。
- **不为了凑覆盖去构造场景。** 先看 ①（下面那栏 `topcut`）能不能在自然跑测里把它逼出来。
- `topcut` 与 `sysrej` **各自对应一道闸门**，两个都为 0 时说明两道都没被考到。
...
同一栏里还有两个数，**各自对应一道闸门，不要混着读**：

| 数 | 非零意味着 | 都为 0 意味着 |
|---|---|---|
| `sysrej` | 标题带里**混进了别的窗口**的元素（状态栏那一类），被窗口归属挡下 | 那道闸门这一跑**没被考到** |
| `topcut` | 状态栏下沿**之上确实有东西**，被几何那一刀切掉 | 那道闸门这一跑**没被考到** |

**`band` 少一个时，这两个数就是"被切掉了"与"这一帧压根没识出它"的区别**——
2026-08-09 第五跑正是卡在这个分不开上：状态栏像素上明明在，落盘证据却说不出是哪一种。

**文本清单只落 logcat**（含界面文字，不进审计 note、不进大脑看得到的信封）：
`adb logcat -d -s gateway-a11y | Select-String "surface title read"`，成功路径也有。
