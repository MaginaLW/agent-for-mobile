...
> **Stale 腿的判据 2026-08-08 换过一次（真机逼出来的）。** 原来要求审计 note 里同时有
> `confirmation=allowed` 与 `context=rechecked`，而**开关打开后这条腿必然终止在「等前台」
> 那一步，走不到「确认后上下文复检」——那条判据结构性不可满足**。现在核的是：
> 真人允许过 + `foreground_wait` 的 `result=timeout` + **`budget_ms` 明显小于生产预算**
> （短预算生效的机械证据）。三条反例逐条钉住，不合并成一个循环。
>
> **顺带修掉一处会得出相反结论的文案**：超时那句原来印的是安全门**问的**那个预算
> （300000ms），而实际只等了 20005ms——**验收单让现场核的恰恰是"短预算有没有生效"，
> 只读那句话会得出"没生效"**。现在印的是**实际等了多久 + 生效预算 + 读了几次 + 最后看到什么**。
...
**新腿怎么跑（`-Legs Reentry`，2026-08-03 接入）：**

1. 前三步与 Allow 完全相同，真人在卡上点「允许本次」。
2. 点完**请把手机放着别动**：debug hook 会把微信切走，runner 随即在外面
   **真实停留 `-ReentryDwellSec` 秒（默认 75，允许 30–240）**，再经自己的 adb 通道
   `am start` 把微信拉回来。屏幕上会先看到桌面、一分多钟后微信自己回到前台。
3. **这条腿慢是设计使然，不是卡住了。** 批准后立刻拉回来的话它几秒就绿，
   却**完全没碰过用户拍板买下的那 5 分钟等待预算**——判据要能看见它要判的东西。
   runner 在停留开始时会用蓝字说明还要等多久。
4. 停留期间 runner 每 5 秒只读采样一次"微信是不是真的不在前台"，结果进 manifest 的
   `legs[].reentry`（`away_samples` / `away_observed` / `dwell_ms` / `restored`）。
   **一次都没采到"已离开"会当场报黄字**：切得不彻底会让整条腿的语义直接落空。

**关于 `configs/gateway-mcp.json` 里的 `"timeout": 420000`——不需要你手工做任何事。**
HTTP MCP 传输的「首字节」计时器默认 **60 秒**，而这条腿的 `press_key` 要阻塞到 400s 量级，
缺这个字段必被砍（现场看到的是「客户端超时、无响应体、无错误码」，与「功能坏了」分不开）。
`-Provision` 每轮会把这份配置连同 token 一起写好，`dispatch` 开跑前再校验一次。

> ⚠️ **别手工往那份文件里加 timeout——加了也会被覆盖。** `-Provision` 每轮重写它，
> 覆盖在前、校验在后。2026-08-08 第二跑就是栽在这条顺序上：闸门加了字段，而**产出这份
> 文件的那条路径没跟着写**，于是四腿在第 1 腿开跑前被自己的闸门拒掉，每轮复现。
> 现在写入方与闸门共用同一份定义（`scripts/lib/gateway-mcp-config.ps1`），
> 并由一条端到端用例钉住「provision 写出的配置必须能通过闸门」。

另一道 300s 的空闲看门狗由**网关在阻塞期间发心跳**顶住（见下）。

**新腿唯一新增的机械判据**（`Assert-P0ReentryForegroundWait`，四条离线反例钉住）：
网关把这段等待写进审计 `note` 的 `foreground_wait=reads=..,waited_ms=..,result=..`，
runner 断言 **`result=reached`、`reads > 1`、`waited_ms ≥ 停留时长的 60%`**。

- `reads=1` → 微信压根没离开过前台，这条腿什么都没验到。
- `waited_ms` 明显偏小 → 被过早拉回或压根没切走。
- **字段整个缺席 → 判失败，不判通过**：装的是不带这段可观测性的旧 APK 时，一条本该证明
  "等过"的腿会静默退化成"什么都没证明"，而它看起来是绿的。

**还要核第二条：传输层心跳真的发出去了。** 审计 note 里的
`sse_heartbeat=beats=N,token=yes` 是网关自报"这次调用往 SSE 上发了几拍进度通知"。
runner 断言 `token=yes` 且 `beats ≥ 1`。**不核这一条的话，"客户端 300s 空闲窗把调用砍了"
与"判据把它挡下了"在现场分不开**——2026-08-08 就是这么烧掉一轮的。三种坏法各有名字：
心跳记录缺席（旧 APK）· `beats=0`（流式没接上，这次只是侥幸没被砍）· `token=no`
（客户端没给 `progressToken`，协议上一拍都发不出去）。

> **传输层这一层为什么长这样（一句话背景）**：客户端对 HTTP MCP 有两道天花板——60s 首字节
> 计时器（靠上面那个 `timeout` 抬高）和 300s 空闲看门狗（靠心跳顶住）。对照实测：
> 同样走 SSE、同样先发首字节，**不发心跳照样在 300s 处被砍；每 60s 发一条，330s 顺利返回**。
...
**读标题这一步自己会说话（批次 4 第三跑之后新增）**：manifest 每腿一栏 `title_read`，
`trail` 逐次列出结论——`resolved` / `no_ocr`（这一帧没跑识别）/ `no_candidate`（跑了但标题带空）
/ `all_rejected`（有元素但没过置信度门槛）/ `no_snapshot`（连屏幕都没拿到）。读取本身带**有界重试**
（最多 4 次、总预算 4s），**只重试"读"，比对方式一个字没放宽**。
现场只需看一件事：**各次结论不同 = 时机问题；逐次相同 = 通道问题。** 前者重跑有用，后者没用。
`title_read` 在 Allow/Stale/Deny 三腿**本来就该缺席**（它们不走重建），只有 Reentry 腿必有。
...
2. **输入栏上方不能有系统浮层。** 上轮联通流量提示压住候选区，把 teardown 判成 `dirty`。
   ...
   **`zen_mode` 要现场读一次再开跑**：vivo 的定时睡眠会自己把它打开，2026-08-09 第四跑
   开跑前实测就是 `1`（凌晨）。**深夜跑测会反复撞到**，别当成一次性的意外。
3. **新腿会让整轮多花一分多钟**（默认停留 75s）。四腿连跑请按 `-Legs Allow,Stale,Deny,Reentry`
   ——Reentry 排在最后是有意的：它是唯一慢的一条，排前面会让人以为跑挂了。
4. **跑真机时不要同时跑离线闸门。** 这台机器可用内存偏低（C 道预热时自报 310MB），
   离线套件会退化成顺序单分片、耗时翻倍，**成片超时与被测代码无关**——而那种超时
   最容易被现场误读成"新判据不稳"。批次 4 尤其要避开：它本来就有一条 5 分钟量级的等待。
5. **上一轮若中途断过线，开跑前先确认输入法已还原。** 断开会**同时**打死拉回步骤与全部收尾，
   `restore_ime` 也在收尾里——于是设备停在网关输入法上进下一轮。详见
   [brain/harness.md](../knowledge/brain/harness.md)。**这一条与断开的原因无关**，
   不要写成"检查线材"：那种检查项永远验不出问题，还会把真因（中途断开）盖住。
...
4. **确认卡与审批通知的存活时间变了**：卡 60s → **90s**，通知跟着到 **105s**（`+15s` slack）。
   ...
   **实测都够，都不改**。现场若发现还得动第四个数，停下来报回来——用户拍的是"5 分钟回来
   还算数"这个行为，不是某一个具体常量。
5. **`E_VERIFY_FAIL` 的判读，2026-08-08 第三跑之后改了（这条以前写错了方向）**：
   - `E_STALE_REF` + 原因含「与已批准的不符」→ 内容或会话真的变了，查为什么变。
   - `E_VERIFY_FAIL` + 原因含「读不回来 / 为空 / 像漏识」→ **先看 `title_read` 那一栏的
     `trail`，不要直接重跑**。各次结论不同 = 时机抖动，重跑有意义；**逐次相同 = 通道问题，
     重跑一百次也一样**。
   - **这条原本写的是"重跑即可，不改任何判据"，而第三跑的三次终态逐字相同——
     按那句话跑，就是拿真机时间去撞一堵不会动的墙。** 当时没有任何一栏能把这两种情形
     分开，`trail` 就是为此补的：判据一个字没动，但现场从第一跑起就知道该往哪边走。
   - **连续多轮都是 `E_VERIFY_FAIL` 且 `trail` 逐次相同**，才是"OCR 这条腿在这一步
     不够用"的信号——那时才谈"标题证据要不要换通道"，而不是谈"比对松一点"。
