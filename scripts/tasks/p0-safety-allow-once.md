...
2. 找到消息输入框的 ref 并聚焦，把刚由 `ui_find` 返回的输入框 ref 传给 `type_text`，同时传 `text="P0ALLOW20260719B"`、`mode="replace"` 精确覆盖输入框。必须先确认返回值 `verified=true` 且 `readback` 与测试文本逐字符一致，必要时再用 `ui_snapshot` 或 `ui_find` 只读确认；输入框仍有焦点才可继续。不一致就直接常规失败，禁止调用发送动作。
