...
function Assert-PrivateAdbTransportLog([string]$State,[int]$ExpectedServerCount){
    ...
    for($lineIndex=0;$lineIndex-lt$transportLines.Count;$lineIndex++){
        ...
        if($raw[0]-ceq'-L'){$serverCount++;Check ($raw.Count-eq4-and$raw[1]-ceq$fields[0]-and$raw[2]-ceq'server'-and$raw[3]-ceq'nodaemon') 'private adb server argv drift';Check (($raw-join$separator)-ceq$adbLines[$lineIndex]) 'private adb server normalized log drift';continue}
...
try{
    ...
    $overlay=@(
      ...
      'docs/contracts/tablet-layout-c1b-sidecar-v1.schema.json','docs/contracts/tablet-c1b-read-only-artifact-proof-v1.schema.json',
...
public static class C1bFakeAdb {
 ...
 static int Server(string[] raw){int port;string socket=raw.Length>1?raw[1]:null;if(raw.Length!=4||raw[0]!="-L"||raw[2]!="server"||raw[3]!="nodaemon"||!SocketPort(socket,out port)||Environment.GetEnvironmentVariable("ADB_SERVER_SOCKET")!=socket){Rejected(raw,"server-endpoint");return 80;}string marker=Path.Combine(S(),"adb-server-stop-"+port.ToString(CultureInfo.InvariantCulture)+".txt");if(File.Exists(marker))File.Delete(marker);TcpListener listener=new TcpListener(IPAddress.Loopback,port);listener.Server.ExclusiveAddressUse=true;listener.Server.SetSocketOption(SocketOptionLevel.Socket,SocketOptionName.ReuseAddress,false);int pid=Process.GetCurrentProcess().Id;try{listener.Start(1);Record(raw,raw,socket);File.AppendAllText(Path.Combine(S(),"adb-server-start.log"),port.ToString(CultureInfo.InvariantCulture)+"\n",new UTF8Encoding(false));Evidence("adb-server-pid.log",port.ToString(CultureInfo.InvariantCulture)+"\u001f"+pid.ToString(CultureInfo.InvariantCulture));while(true){if(File.Exists(marker)){string value=null;try{value=File.ReadAllText(marker,Encoding.UTF8);}catch(IOException){}if(value==socket){File.Delete(marker);break;}}Thread.Sleep(10);}}finally{listener.Stop();}File.AppendAllText(Path.Combine(S(),"adb-server-exit.log"),port.ToString(CultureInfo.InvariantCulture)+"\n",new UTF8Encoding(false));Evidence("adb-server-exit-pid.log",port.ToString(CultureInfo.InvariantCulture)+"\u001f"+pid.ToString(CultureInfo.InvariantCulture));return 0;}
...
'@
    ...
    Check ($serverSnapshot.Start.Count-eq6-and$serverSnapshot.Status.Count-eq6-and$serverSnapshot.Kill.Count-eq4-and$serverSnapshot.Exit.Count-eq6) 'official auto-start aggregate private-server lifecycle drift'
