...
'@
    ...
    Assert-True ($e2e.real_adb_call_count-eq0-and$e2e.fake_adb_call_count-eq222-and$e2e.fake_adb_valid_call_count-eq214-and$e2e.fake_adb_rejected_endpoint_call_count-eq8-and$e2e.private_adb_server_start_count-eq8-and$e2e.private_adb_server_status_count-eq7-and$e2e.private_adb_server_kill_count-eq4-and$e2e.private_adb_server_exit_count-eq7-and$e2e.private_adb_device_call_count-eq195-and$e2e.private_adb_t0_call_count-eq4-and$e2e.runner_process_call_count-eq9-and$e2e.fake_gradle_call_count-eq8-and$e2e.fake_signer_call_count-eq12-and$e2e.build_environment_input_count-eq42) 'E2E exact fake-tool aggregate drift'
    foreach($coverage in @('runner_e2e_success','runner_e2e_result_control_abort','runner_e2e_malformed_abort_fail_closed','runner_e2e_tamper_abort','runner_e2e_early_private_adb_server_exit','runner_e2e_early_device_lease_cleanup_failure','runner_e2e_early_private_adb_status_client_exit')){Assert-True ($e2e.coverage-ccontains$coverage) "E2E coverage 缺失 $coverage";$passed.Add($coverage)}
