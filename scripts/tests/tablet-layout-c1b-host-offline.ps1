...
function New-SidecarFixture {
    $h='sha256:'+'d'*64;$impl=[ordered]@{};foreach($name in @('runner_sha256','c1b_library_sha256','c1a_low_level_library_sha256','t0_runner_sha256','t0_library_sha256','t0_adb_sidecar_cmd_sha256','t0_adb_sidecar_script_sha256','validator_sha256','native_path_validator_sha256','observation_schema_sha256','sidecar_schema_sha256','android_model_sha256','android_probe_sha256','android_source_sha256','android_provider_sha256','android_protocol_sha256','android_coordinator_sha256','android_controller_sha256','android_context_sha256','android_pending_registry_sha256','debug_manifest_sha256','gateway_build_gradle_sha256')){$impl[$name]=$h}
    return [ordered]@{schema='tablet-layout-c1b-sidecar/v1';run_id='tl1-c1b-test';completed_at_utc='2026-08-26T01:02:03.1234567Z';expected_commit_sha=('a'*40);capture_scope='pure_a11y';provenance_strategy='clean_content_provider_independently_attested';static_read_only_policy_version='tl1-c1b-read-only/v1';implementation_hashes=$impl
        transport=[ordered]@{trust_root='android_sdk_platform_tools';canonical_relative_path='platform-tools/adb.exe';sdk_roots_equal=$true;executable_sha256_before=$h;executable_sha256_after=$h;version_output_sha256_before=$h;version_output_sha256_after=$h;protocol_version='1.0.41';package_version='36.0.0-13206524';installed_as_canonical=$true}
        ...
        artifacts=[ordered]@{upstream_t0=[ordered]@{relative_path='upstream-t0-v5.json';sha256=$h};observation=[ordered]@{relative_path='tablet-layout-observation-c1b-v1.json';sha256=$h};validation=[ordered]@{relative_path='tablet-layout-observation-validation-c1b-v1.json';sha256=$h}}
...
'@
    ...
    Pass single_c1_c2_result {Assert-True ([regex]::Matches($runnerRaw,'Read-C1bControl content_c1').Count-eq1) 'content_c1 调用数漂移';Assert-True ([regex]::Matches($runnerRaw,'Read-C1bControl content_c2').Count-eq1) 'content_c2 调用数漂移';Assert-True ([regex]::Matches($runnerRaw,'-Name content_result').Count-eq1) 'content_result 调用数漂移';Assert-True ($runnerRaw-match'recapture_count=0') 'recapture 结论缺失'}
    ...
    Pass sdk_adb_trust_root {Assert-True ($runnerRaw-match'Get-TL1C1bAdbTrustBinding'-and$runnerRaw-match'ANDROID_SDK_ROOT'-and$runnerRaw-match'ANDROID_HOME') 'runner 缺失 canonical SDK adb trust root';$s=New-SidecarFixture;$s.transport.executable_sha256_after='sha256:'+'e'*64;Assert-Throws {Assert-TL1C1bSidecarCrossBindings ([pscustomobject](($s|ConvertTo-Json -Depth 30)|ConvertFrom-Json -DateKind String))} 'adb executable hash drift 未拒绝'}
    ...
    Pass sidecar_closed_schema {$s=New-SidecarFixture;$raw=$s|ConvertTo-Json -Depth 30 -Compress;Assert-True ($raw|Test-Json -SchemaFile $SidecarSchema) 'valid sidecar schema 拒绝';$s.extra=$true;Assert-True (-not($s|ConvertTo-Json -Depth 30 -Compress|Test-Json -SchemaFile $SidecarSchema -ErrorAction SilentlyContinue)) 'sidecar extra 未拒绝'}
    Pass sidecar_cross_binding {$s=New-SidecarFixture;Assert-TL1C1bSidecarCrossBindings ([pscustomobject](($s|ConvertTo-Json -Depth 30)|ConvertFrom-Json -DateKind String));foreach($path in @('apk.local_sha256_after','provider.embedded_git_head','capture.host_wait_ms','capture.c1_requests_accepted','upstream_t0.original_sha256','claims.wechat_window_ownership_verified')){$x=($s|ConvertTo-Json -Depth 30)|ConvertFrom-Json -DateKind String;switch($path){'apk.local_sha256_after'{$x.apk.local_sha256_after='sha256:'+'e'*64};'provider.embedded_git_head'{$x.provider.embedded_git_head='e'*40};'capture.host_wait_ms'{$x.capture.host_wait_ms=899};'capture.c1_requests_accepted'{$x.capture.c1_requests_accepted=2};'upstream_t0.original_sha256'{$x.upstream_t0.original_sha256='sha256:'+'e'*64};default{$x.claims.wechat_window_ownership_verified=$false}};Assert-Throws {Assert-TL1C1bSidecarCrossBindings $x} "cross spoof 未拒绝: $path"}}
    Pass release_debug_boundary {$manifest=Get-Content (Join-Path $RepoRoot 'app\gateway\src\debug\AndroidManifest.xml') -Raw;Assert-True ($manifest-match[regex]::Escape($script:TL1C1bAuthority)) 'debug provider 缺失';$mainManifest=Get-Content (Join-Path $RepoRoot 'app\gateway\src\main\AndroidManifest.xml') -Raw;Assert-True ($mainManifest-cnotmatch[regex]::Escape($script:TL1C1bAuthority)) 'main manifest 暴露 C1b authority'}
    ...
    Pass failure_atomic_cleanup {$c1aRaw=Get-Content $C1aLibrary -Raw;Assert-True ($runnerRaw-match'Write-C1bFailureEvidence'-and$runnerRaw-match'not\$sessionConsumed'-and$runnerRaw-match'content_abort'-and$c1aRaw-match'Remove-Item -LiteralPath \$destinationFull') 'failure/abort 原子边界缺失'}
    ...
    Pass summary_deception_rejection {
        $now=[DateTime]::UtcNow.ToString("yyyy-MM-dd'T'HH:mm:ss.fffffff'Z'");$summary=[ordered]@{schema='tablet-layout-c1b-host-offline-summary/v1';gate_run_id=$GateRunId;started_at_utc=$now;completed_at_utc=$now;status='passed';fake_adb=$true;real_adb_call_count=[long]0;test_case_count=[long]$script:TL1C1bRequiredOfflineCoverage.Count;coverage_case_count=[long]$script:TL1C1bRequiredOfflineCoverage.Count;coverage=$script:TL1C1bRequiredOfflineCoverage;claims=[ordered]@{runtime_origin_verified=$false;runtime_evidence=$false;layout_accepted=$false;wechat_layout_verified=$false;editor_action_ready=$false;p0_capability='unsupported';execution_grant=$false}}
        [void](ConvertFrom-TL1C1bOfflineSummary ($summary|ConvertTo-Json -Depth 8 -Compress) $GateRunId)
        foreach($mutation in @('extra','fake','count','claim')){$x=($summary|ConvertTo-Json -Depth 8)|ConvertFrom-Json;switch($mutation){extra{$x|Add-Member extra $true};fake{$x.fake_adb=$false};count{$x.test_case_count--};claim{$x.claims.runtime_evidence=$true}};Assert-Throws {ConvertFrom-TL1C1bOfflineSummary ($x|ConvertTo-Json -Depth 8 -Compress) $GateRunId} "summary $mutation 欺骗未拒绝"}
    }
